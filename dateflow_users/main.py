...
# Handle Allowed Origins (Comma separated string to list)
...
async def lifespan(app: FastAPI):
    ...
    if not FRONTEND_DIR.exists():
        ...
        # We don't raise an error here to allow the API to start even if frontend is missing (e.g. backend-only mode)
    ...
    if ENVIRONMENT == "production":
        ...
        if missing_vars:
            # We log a warning but don't crash, in case some features are optional
            logger.warning(f"⚠️ Production Warning: Missing env vars: {', '.join(missing_vars)}")
    ...
    try:
        ...
        # In production, DB failure is usually fatal
        if ENVIRONMENT == "production":
...
# Trust headers from proxies (Render/Heroku/Nginx)
app.add_middleware(
...
# CRITICAL FIX: explicit prefixes here ensure frontend calls to /api/auth/... work
