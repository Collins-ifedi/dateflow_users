...
async def create_verification(
    ...
    and queues the email via Brevo.
    """
    code = f"{random.randint(100000, 999999)}"
    
    # FIX: Calculate expiry time (UTC) to prevent "NOT NULL constraint" DB error
    ...
    verification = Verification(
        ...
        expires_at=expires_at,  # This field was missing in previous versions
        is_verified=False
    )
    db.add(verification)
    await db.commit()
...
async def login(
    ...
    # Use the new verify_password function (bcrypt based)
