...
import vonage  # NEW: Vonage SDK for SMS
...
# ------------------------------------------------------
# SMS CONFIG (Vonage) - NEW
# ------------------------------------------------------
VONAGE_API_KEY = os.getenv("VONAGE_API_KEY")
VONAGE_API_SECRET = os.getenv("VONAGE_API_SECRET")
VONAGE_SENDER_NAME = os.getenv("VONAGE_SENDER_NAME", "DataFlow") # Alphanumeric sender ID
...
def hash_password(password: str) -> str:
    """
    Hashes a password using bcrypt directly to avoid Passlib/Python 3.13 issues.
    """
...
def verify_password(password: str, password_hash: str) -> bool:
    ...
    Safely handles encoding to prevent runtime errors.
    """
    try:
        pwd_bytes = password.encode('utf-8')
...
def send_email_via_brevo(to_email: str, subject: str, html_content: str):
    ...
    send_smtp_email = sib_api_v3_sdk.SendSmtpEmail(
        to=[{"email": to_email}],
        sender={"name": "DataFlow Support", "email": "no-reply@dataflow.com"},
        subject=subject,
...
def send_sms_via_vonage(to_phone: str, message_text: str):
    ...
    # Basic cleanup: Ensure number doesn't have leading '+' if Vonage prefers it without, 
    # though Vonage generally accepts E.164 (e.g., 447700900000). 
    # Assuming `to_phone` is stored in valid international format in DB.
...
async def create_verification(
    ...
    """
    Generates a 6-digit OTP, saves it to DB with an expiration date,
    and queues the notification (Email or SMS).
    """
    ...
    # ------------------------------------------------
    # HANDLE SMS VERIFICATION (VONAGE)
    # ------------------------------------------------
...
async def verify_account(
    ...
    # Check Expiry (Compare timezone-aware UTC times)
...
async def recover_password(
    ...
    if not user:
        ...
        # Return fake success or 404 depending on your security policy. 
        # For better UX in this context, we'll return 404 for debugging.
        raise HTTPException(status_code=404, detail="User not found")
...
async def reset_password(
    ...
    # Delete the used verification record to prevent reuse
